...
const Camera = (() => {
  ...
  function lookAt(m, eye, center, up) {
    ...
    if (zl < 1e-8) { identity(m); return m; }
    const zinvL = 1/zl;
    const Zx = zx*zinvL, Zy = zy*zinvL, Zz = zz*zinvL;
    ...
    if (xl < 1e-8) xl = 1;
    const xinvL = 1/xl;
    const XXx = Xx*xinvL, XXy = Xy*xinvL, XXz = Xz*xinvL;

    const Yx = Zy*XXz - Zz*XXy;
    const Yy = Zz*XXx - Zx*XXz;
    const Yz = Zx*XXy - Zy*XXx;

    m[0]=XXx; m[1]=Yx; m[2]=Zx;  m[3]=0;
    m[4]=XXy; m[5]=Yy; m[6]=Zy;  m[7]=0;
    m[8]=XXz; m[9]=Yz; m[10]=Zz; m[11]=0;
  ...
  function translateX(m, tx) {
    const r=identity(mat4()); r[12]=tx;
    return multiply(mat4(),m,r);
  }

  // Normal matrix (upper-left 3×3 inverse-transpose of model)
  ...
  // ── Quaternion helpers ────────────────────────────────────
  function quat(x=0,y=0,z=0,w=1){ return new Float32Array([x,y,z,w]); }
  ...
  function create(canvas) {
    ...
    canvas.addEventListener('touchmove', e=>{
      if(e.touches.length===1 && dragging){
        ...
        const speed=0.005;
        rotQuat=qMultiply(qMultiply(axisAngleQuat(0,1,0,dx*speed),axisAngleQuat(1,0,0,dy*speed)),rotQuat);
        lastX=e.touches[0].clientX;
        lastY=e.touches[0].clientY;
      } else if(e.touches.length===2){
        const dx=e.touches[0].clientX-e.touches[1].clientX;
    ...
    return { update, getViewMatrix, getProjectionMatrix, getEyePosition, getRotMatrix,
             mat4, identity, multiply, scale, normalMatrix };
  }
